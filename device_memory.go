...
func (m *DeviceMemory) MapMemory(offset int, size int) (unsafe.Pointer, VKng.Result, error) {
	var data unsafe.Pointer
	res := VKng.Result(C.vkMapMemory(m.device, m.handle, C.VkDeviceSize(offset), C.VkDeviceSize(size), 0, &data))
	err := res.ToError()
	if err != nil {
		return nil, res, err
	}
...
func (m *DeviceMemory) WriteData(offset int, data interface{}) (VKng.Result, error) {
	...
	buf := &bytes.Buffer{}
	err = binary.Write(buf, VKng.ByteOrder, data)
	if err != nil {
