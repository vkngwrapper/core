...
func CreateInstance(allocator cgoalloc.Allocator, options *InstanceOptions) (*Instance, VKng.Result, error) {
	...
	res := VKng.Result(C.vkCreateInstance((*C.VkInstanceCreateInfo)(createInfo), nil, &instanceHandle))
	err = res.ToError()
	if err != nil {
		return nil, res, err
	}
...
func (i *Instance) PhysicalDevices(allocator cgoalloc.Allocator) ([]*PhysicalDevice, VKng.Result, error) {
	count := (*C.uint32_t)(allocator.Malloc(int(unsafe.Sizeof(C.uint32_t(0)))))
	defer allocator.Free(unsafe.Pointer(count))

	res := VKng.Result(C.vkEnumeratePhysicalDevices(i.handle, count, nil))
	err := res.ToError()
	if err != nil {
		return nil, res, err
	}
	...
	res = VKng.Result(C.vkEnumeratePhysicalDevices(i.handle, count, (*C.VkPhysicalDevice)(allocatedHandles)))
	err = res.ToError()
	if err != nil {
		return nil, res, err
	}
